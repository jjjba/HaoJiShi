package com.haojishi.service.weChat;

import com.fasterxml.jackson.core.type.TypeReference;
import com.fasterxml.jackson.databind.ObjectWriter;
import com.fasterxml.jackson.dataformat.xml.XmlMapper;
import com.haojishi.util.GxCommon;
import com.haojishi.util.SignUtils;
import com.haojishi.util.WxApiUtil;
...
public class WechatOrderService {
    ...
    public Map<String, Object> merchantPay(String appId, String mchId, String body, String detail, String attach,
                                           ...
                                           boolean limitPay, String openId, String apiKey, String notifyUrl) {
        log.debug("appId = [" + appId + "], mchId = [" + mchId + "], body = [" + body + "], detail = [" + detail + "], attach = [" + attach + "], outTradeNo = [" + outTradeNo + "], totalFee = [" + totalFee + "], spbillCreateIp = [" + spbillCreateIp + "], timeStart = [" + timeStart + "], timeExpire = [" + timeExpire + "], goodsTag = [" + goodsTag + "], limitPay = [" + limitPay + "], openId = [" + openId + "], apiKey = [" + apiKey + "]");
    ...
    public Map<String, Object> serviceProviderPay(String appId, String mchId, String subAppId, String subMchId, String body, String detail, String attach,
                                                  String outTradeNo, Integer totalFee, String spbillCreateIp, String timeStart, String timeExpire, String goodsTag,
                                                  String productId, boolean limitPay, String subOpenId, String apiKey, String notifyUrl) {

        log.debug("appId = [" + appId + "], mchId = [" + mchId + "], subAppId = [" + subAppId + "], subMchId = [" + subMchId + "]," +
                " body = [" + body + "], detail = [" + detail + "], attach = [" + attach + "], outTradeNo = [" + outTradeNo + "], totalFee =" +
                " [" + totalFee + "], spbillCreateIp = [" + spbillCreateIp + "], timeStart = [" + timeStart + "], timeExpire = [" + timeExpire + "], " +
                "goodsTag = [" + goodsTag + "], productId = [" + productId + "], limitPay = [" + limitPay + "], subOpenId = [" + subOpenId + "], " +
                "apiKey = [" + apiKey + "], notifyUrl = [" + notifyUrl + "]");
        Map<String, Object> data = null;
        try {
            TreeMap<String, Object> params = new TreeMap<>();
            // 公众账号ID
...
//            params.put("openid", openId);
    ...
    private Map<String, Object> process(TreeMap<String, Object> params, String apiKey) {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(sign)) {
                ...
                String result = Request.Post(WxApiUtil.WX_PRE_ORDER_URL)
     ...
     * @return 创建结果
     */
    public Map<String, Object> process(TreeMap<String, Object> params, String apiKey, String reqUrl) {
        Map<String, Object> data = new HashMap<>();
        try {
            String sign = WxPayUtil.createSign(params, apiKey, "UTF-8");
            ...
            if (StringUtils.isNotBlank(sign)) {
                ...
                log.debug("响应结果：{}", data);
                if (!SignUtils.checkParam(data, apiKey)) {
                    log.error("创建订单失败，{}", temp);
                } else {
                    if ("0".equals(data.get("status")) && "0".equals(data.get("result_code"))) {
                        String pay_info = data.get("pay_info") + "";
